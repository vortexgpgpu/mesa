 ...
 * (0 or 1 = single-sample), and `bgra` the colour attachment's channel order
 * (false = red in the low byte, true = blue); all three are ignored for
 * VS/compute. Two fragment shaders differing only in these are different
 * kernels, which is why they are translation inputs rather than draw-time
 * arguments. */
